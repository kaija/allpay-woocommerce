...
/*
 * Plugin Name:  歐付寶 - Barcode 收款模組
 * Plugin URI: http://www.ecbank.com.tw/module/index.php
 * Description: AllPay Payment Gateway by Barcode for WooCommerce
 * Version: 1.0
 * Author: AllPay Financial Information Service Co., Ltd.
 * Author URI: http://www.allpay.com.tw
 */

add_action('plugins_loaded', 'AllPay_Barcode_Plugin_Init', 0);

function AllPay_Barcode_Plugin_Init() {
    // Make sure WooCommerce is setted.
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

    class WC_Gateway_Barcode extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = "barcode";
            $this->method_title = __('AllPay (超商條碼)', 'woocommerce');
            ...
            $this->return_url = add_query_arg('wc-api', 'WC_Gateway_Barcode', home_url('/')) . '&callback=return';
            $this->client_back_url = trailingslashit(home_url());
            $this->order_result_url = add_query_arg('wc-api', 'WC_Gateway_Barcode', home_url('/'));
            $this->payment_info_url = add_query_arg('wc-api', 'WC_Gateway_Barcode', home_url('/')) . '&callback=payment_info';
            // Logs
            if ('yes' == $this->debug) {
        ...
        function init_form_fields() {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default' => __('AllPay - Barcode Payment Gateway', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('Description', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('This controls the description which the user sees during checkout.', 'woocommerce'),
                    'default' => __('Pay securely by Barcode through AllPay Secure Servers.', 'woocommerce')
                ),
                'merchant_id' => array(
        ...
        public function admin_options() {
            echo '<h3>' . __('AllPay - Barcode Payment Gateway', 'woocommerce') . '</h3>';
        ...
        public function receipt_page($order) {
            ...
            if ($oOrder) {
                ...
                try {
                    ...
                    array_push($oPayment->Send['Items'], array('Name' => '商品一批', $oPayment->Send['TotalAmount'], 'Currency' => get_woocommerce_currency(), 'Quantity' => 1, 'URL' => ''));

                    $oPayment->SendExtend['ExpireDate'] = 3;
                    $oPayment->SendExtend['PaymentInfoURL'] = $this->payment_info_url;
                    //$oPayment->SendExtend['ClientRedirectURL'] = $this->order_result_url;
            ...
            try {
                ...
                if (sizeof($arFeedback) > 0) {
                    ...
                    if ($deTradeAmount == $deTotalAmount) {
                        ...
                        if ($szReturnCode == 1 || $szReturnCode == 10100073 || $szReturnCode == 800) {
                            $szComment = "$szReturnCode: $szReturnMessgae";
                            // 取得 Barcode 虛擬帳號資料(紀錄於歷程中)。
                            if ($szReturnCode == 10100073 && $szOrderStatus == 'pending') {
                                ...
                                $szComment = sprintf('Payment Method: %s<br />Trade Time: %s<br />Trade Code: %s<br />Payment Deadline: %s<br />BARCODE 1: %s<br />BARCODE 2: %s<br />BARCODE 3: %s<br />',
                                        $szPaymentType, $szTradeDate, $szBankCode, $szExpireDate, $szBarcode1, $szBarcode2, $szBarcode3);

                                $oOrder->add_order_note($szComment);
                            }
                            // 更新訂單資料與新增訂單歷程。
                            else if ($szOrderStatus == 'pending') {
     ...
     * */
    function woocommerce_add_allpay_barcode_plugin($methods) {
        $methods[] = 'WC_Gateway_Barcode';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_allpay_barcode_plugin');
}
