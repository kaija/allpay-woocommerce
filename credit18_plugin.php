...
/*
 * Plugin Name:  歐付寶 - Credit Card 分 18 期收款模組
 * Plugin URI: http://www.ecbank.com.tw/module/index.php
 * Description: AllPay Payment Gateway by Credit Card in 18 Installments for WooCommerce
 * Version: 1.0
 * Author: AllPay Financial Information Service Co., Ltd.
 * Author URI: http://www.allpay.com.tw
 */

add_action('plugins_loaded', 'AllPay_Credit18_Plugin_Init', 0);

function AllPay_Credit18_Plugin_Init() {
    // Make sure WooCommerce is setted.
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

    class WC_Gateway_Credit18 extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = "credit18";
            $this->method_title = __('AllPay (信用卡 18 期)', 'woocommerce');
            ...
            $this->return_url = add_query_arg('wc-api', 'WC_Gateway_Credit18', home_url('/')) . '&callback=return';
            $this->client_back_url = trailingslashit(home_url());
            $this->order_result_url = add_query_arg('wc-api', 'WC_Gateway_Credit18', home_url('/'));
            $this->payment_info_url = add_query_arg('wc-api', 'WC_Gateway_Credit18', home_url('/')) . '&callback=payment_info';
            // Logs
            if ('yes' == $this->debug) {
        ...
        function init_form_fields() {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default' => __('AllPay - Credit Card in 18 installments Payment Gateway', 'woocommerce')
                ),
                'description' => array(
        ...
        public function receipt_page($order) {
            ...
            if ($oOrder) {
                ...
                try {
                    ...
                    $oPayment->SendExtend['CreditInstallment'] = 18;
     ...
     * */
    function woocommerce_add_allpay_credit18_plugin($methods) {
        $methods[] = 'WC_Gateway_Credit18';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_allpay_credit18_plugin');
}
