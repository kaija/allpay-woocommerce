...
/*
 * Plugin Name:  歐付寶 - Credit Card 分 24 期收款模組
...
add_action('plugins_loaded', 'AllPay_Credit24_Plugin_Init', 0);

function AllPay_Credit24_Plugin_Init() {
    // Make sure WooCommerce is setted.
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

    class WC_Gateway_Credit24 extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = "credit24";
            $this->method_title = __('AllPay (信用卡 24 期)', 'woocommerce');
            ...
            $this->return_url = add_query_arg('wc-api', 'WC_Gateway_Credit24', home_url('/')) . '&callback=return';
            $this->client_back_url = trailingslashit(home_url());
            $this->order_result_url = add_query_arg('wc-api', 'WC_Gateway_Credit24', home_url('/'));
            $this->payment_info_url = add_query_arg('wc-api', 'WC_Gateway_Credit24', home_url('/')) . '&callback=payment_info';
            // Logs
            if ('yes' == $this->debug) {
     ...
     * */
    function woocommerce_add_allpay_credit24_plugin($methods) {
        $methods[] = 'WC_Gateway_Credit24';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_allpay_credit24_plugin');
}
