...
/*
 * Plugin Name:  歐付寶 - CVS 收款模組
 * Plugin URI: http://www.ecbank.com.tw/module/index.php
 * Description: AllPay Payment Gateway by CVS for WooCommerce
 * Version: 1.0
 * Author: AllPay Financial Information Service Co., Ltd.
 * Author URI: http://www.allpay.com.tw
 */

add_action('plugins_loaded', 'AllPay_CVS_Plugin_Init', 0);

function AllPay_CVS_Plugin_Init() {
    // Make sure WooCommerce is setted.
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

    class WC_Gateway_CVS extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = "cvs";
            $this->method_title = __('AllPay (超商代碼)', 'woocommerce');
            ...
            $this->return_url = add_query_arg('wc-api', 'WC_Gateway_CVS', home_url('/')) . '&callback=return';
            $this->client_back_url = trailingslashit(home_url());
            $this->order_result_url = add_query_arg('wc-api', 'WC_Gateway_CVS', home_url('/'));
            $this->payment_info_url = add_query_arg('wc-api', 'WC_Gateway_CVS', home_url('/')) . '&callback=payment_info';
            // Logs
            if ('yes' == $this->debug) {
        ...
        function init_form_fields() {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default' => __('AllPay - CVS Payment Gateway', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('Description', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('This controls the description which the user sees during checkout.', 'woocommerce'),
                    'default' => __('Pay securely by CVS through AllPay Secure Servers.', 'woocommerce')
                ),
                'merchant_id' => array(
        ...
        public function admin_options() {
            echo '<h3>' . __('AllPay - CVS Payment Gateway', 'woocommerce') . '</h3>';
                ...
                if (sizeof($arFeedback) > 0) {
                    ...
                    if ($deTradeAmount == $deTotalAmount) {
                        ...
                        if ($szReturnCode == 1 || $szReturnCode == 2 || $szReturnCode == 800) {
                            ...
                            // 取得 CVS 虛擬帳號資料(紀錄於歷程中)。
                            if ($szReturnCode == 2 && $szOrderStatus == 'pending') {
                                ...
                                $szComment = sprintf('Payment Method: %s<br />Trade Time: %s<br />Trade Code: %s<br />Payment Deadline: %s<br />BARCODE 1: %s<br />BARCODE 2: %s<br />BARCODE 3: %s<br />', $szPaymentType, $szTradeDate, $szBankCode, $szExpireDate, $szBarcode1, $szBarcode2, $szBarcode3);
     ...
     * */
    function woocommerce_add_allpay_cvs_plugin($methods) {
        $methods[] = 'WC_Gateway_CVS';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_allpay_cvs_plugin');
}
