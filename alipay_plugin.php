...
/*
 * Plugin Name:  歐付寶 - Alipay 收款模組
 * Plugin URI: http://www.ecbank.com.tw/module/index.php
 * Description: AllPay Payment Gateway by Alipay for WooCommerce
 * Version: 1.0
 * Author: AllPay Financial Information Service Co., Ltd.
 * Author URI: http://www.allpay.com.tw
 */

add_action('plugins_loaded', 'AllPay_Alipay_Plugin_Init', 0);

function AllPay_Alipay_Plugin_Init() {
    ...
    class WC_Gateway_Alipay extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = "alipay";
            $this->method_title = __('AllPay (支付寶)', 'woocommerce');
            ...
            $this->return_url = add_query_arg('wc-api', 'WC_Gateway_Alipay', home_url('/')) . '&callback=return';
            $this->client_back_url = trailingslashit(home_url());
            $this->order_result_url = add_query_arg('wc-api', 'WC_Gateway_Alipay', home_url('/'));
            $this->payment_info_url = add_query_arg('wc-api', 'WC_Gateway_Alipay', home_url('/')) . '&callback=payment_info';
            // Logs
            if ('yes' == $this->debug) {
        ...
        function init_form_fields() {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default' => __('AllPay - Alipay Payment Gateway', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('Description', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('This controls the description which the user sees during checkout.', 'woocommerce'),
                    'default' => __('Pay securely by Alipay through AllPay Secure Servers.', 'woocommerce')
                ),
                'merchant_id' => array(
        ...
        public function admin_options() {
            echo '<h3>' . __('AllPay - Alipay Payment Gateway', 'woocommerce') . '</h3>';
        ...
        public function receipt_page($order) {
            ...
            if ($oOrder) {
                ...
                try {
                    ...
                    $oPayment->SendExtend['Email'] = $oOrder->billing_email;
                    $oPayment->SendExtend['PhoneNo'] = $oOrder->billing_phone;
                    $oPayment->SendExtend['UserName'] = $oOrder->billing_first_name . ' ' . $oOrder->billing_last_name;
     ...
     * */
    function woocommerce_add_allpay_alipay_plugin($methods) {
        $methods[] = 'WC_Gateway_Alipay';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_allpay_alipay_plugin');
}
