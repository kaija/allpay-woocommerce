...
/*
 * Plugin Name:  歐付寶 - Tenpay 收款模組
 * Plugin URI: http://www.ecbank.com.tw/module/index.php
 * Description: AllPay Payment Gateway by Tenpay for WooCommerce
 * Version: 1.0
 * Author: AllPay Financial Information Service Co., Ltd.
 * Author URI: http://www.allpay.com.tw
 */

add_action('plugins_loaded', 'AllPay_Tenpay_Plugin_Init', 0);

function AllPay_Tenpay_Plugin_Init() {
    // Make sure WooCommerce is setted.
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

    class WC_Gateway_Tenpay extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = "tenpay";
            $this->method_title = __('AllPay (財付通)', 'woocommerce');
            ...
            $this->return_url = add_query_arg('wc-api', 'WC_Gateway_Tenpay', home_url('/')) . '&callback=return';
            $this->client_back_url = trailingslashit(home_url());
            $this->order_result_url = add_query_arg('wc-api', 'WC_Gateway_Tenpay', home_url('/'));
            $this->payment_info_url = add_query_arg('wc-api', 'WC_Gateway_Tenpay', home_url('/')) . '&callback=payment_info';
            // Logs
            if ('yes' == $this->debug) {
        ...
        function init_form_fields() {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default' => __('AllPay - Tenpay Payment Gateway', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('Description', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('This controls the description which the user sees during checkout.', 'woocommerce'),
                    'default' => __('Pay securely by Tenpay through AllPay Secure Servers.', 'woocommerce')
                ),
                'merchant_id' => array(
        ...
        public function admin_options() {
            echo '<h3>' . __('AllPay - Tenpay Payment Gateway', 'woocommerce') . '</h3>';
     ...
     * */
    function woocommerce_add_allpay_tenpay_plugin($methods) {
        $methods[] = 'WC_Gateway_Tenpay';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'woocommerce_add_allpay_tenpay_plugin');
}
